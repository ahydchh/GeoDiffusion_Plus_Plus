...
from mmdet.models.losses import iou_loss
...
def parse_args():
    ...
    parser.add_argument(
        "--reward_scale", type=float, default=1.0, help="Scale divided for grad loss value."
    )
...
def process_img(imgs:list, mean, std, height, width):
    processed_imgs = []
    for img in imgs:
        processed_img = img[[2,1,0]]
        processed_img = (processed_img * std + mean).permute(1, 2, 0)
        processed_img.require_grad = True
        processed_img = processed_img.to(dtype = torch.float32)
        # processed_img = Image.fromarray(processed_img.astype(np.uint8),mode = 'RGB')
        # processed_img = processed_img.resize((height, width))
...
def test_pipeline(img):
    if not isinstance(img, torch.Tensor):
        ...
        img = torch.tensor(numpy_img,requires_grad=True,dtype=torch.float32)

    results = {}
    results['filename'] = None
    results['ori_filename'] = None
    results['img'] = img
    results['img_shape'] = img.shape # (427, 640, 3)
    ...
    img = trans_F.resize(results['img'], (new_size[1], new_size[0]), interpolation=InterpolationMode.BILINEAR)
    ...
    results['img'] = trans_F.normalize(results['img'],mean,std)
    ...
    pad_h = int((img.shape[1] + 32 - 1) // 32 * 32)
    pad_w = int((img.shape[2] + 32 - 1) // 32 * 32)
    # import pdb; pdb.set_trace()
    padded_img = trans_F.pad(img, (0, 0, 15, 0), padding_mode='constant', fill=0) #(800,1216,3)
    results['pad_shape'] = (padded_img.shape[1], padded_img.shape[2], padded_img.shape[0])
    results['img'] = padded_img
    results['pad_fixed_size'] = None
    results['pad_size_divisor'] = 32
    ...
    # dot=make_dot(data['img'][0].data, params={"img": img})
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                mean = torch.tensor([127.5, 127.5, 127.5],device=batch["pixel_values"].device).view(3, 1, 1)
                std = torch.tensor([127.5, 127.5, 127.5],device=batch["pixel_values"].device).view(3, 1, 1)

                h = train_dataset.data_infos[batch['idxs'][0]]["height"]
                w = train_dataset.data_infos[batch['idxs'][0]]["width"]

                processed_gts = process_img(batch["pixel_values"], mean, std,h,w)
                processed_ims = process_img(image, mean, std,h,w)

                gt_datas = [test_pipeline(processed_gt) for processed_gt in processed_gts]
                im_datas = [test_pipeline(processed_im) for processed_im in processed_ims]
       
                _,gt_outputs,valid_masks = inference_detector(reward_model,gt_datas,tensor=True)
                _,outputs,_ = inference_detector(reward_model,im_datas,tensor=True)

                huber_loss_fn = nn.SmoothL1Loss()
                l2_loss_fn = nn.MSELoss()
                assert len(outputs)==len(gt_outputs)
                reward_losses = []
                for batch_output , batch_gt_output , batch_valid_mask in zip(outputs,gt_outputs,valid_masks):
                    pred_bbox = batch_output[0]
                    gt_bbox = batch_gt_output[0]
                    valid_mask = batch_valid_mask.reshape(-1)
                    pred_bbox = pred_bbox[valid_mask]
                    gt_bbox = gt_bbox[valid_mask]
                    assert pred_bbox.size()==gt_bbox.size()
                    bbox_loss = huber_loss_fn(pred_bbox[:,:4]+1e-10,gt_bbox[:,:4]+1e-10)
                    centerness_loss = l2_loss_fn(pred_bbox[:,4]+1e-10,gt_bbox[:,4]+1e-10)
                    loss = bbox_loss + centerness_loss * args.centerness_weight
                    reward_losses.append(loss)
                reward_loss = torch.stack(reward_losses)


                timestep_mask = (args.min_timestep_rewarding <= timesteps.reshape(-1, 1)) & (timesteps.reshape(-1, 1) <= args.max_timestep_rewarding)

                #TODO Reawrd Loss: (B, H, W)  =>  (B)
                ...
                loss = pretrain_loss + reward_loss * args.reward_scale
                """
                Losses
                """
