...
def run(
    ...
    for group_idx in range(5):
        ...
        for path, im, im0s, vid_cap, s in tqdm(dataset):
            ...
            # Process predictions
            save_one_json(det, results, Path(path), class_map)
                
        # save_path = f"test_dir/{source.split('/')[-4]}_{group_idx}.json"
        # print(f"=== saving to {save_path} ===")
        # with open(save_path, "w") as f:
        #     json.dump(results, f, indent=4)  # 使用 indent=4 让 JSON 文件更易读
        coco = COCO(opt.anno)
        map_results = COCOResults('bbox')
        # import pdb;pdb.set_trace()
        coco_dt = coco.loadRes(results)
        coco_eval = COCOeval(coco, coco_dt, 'bbox')
        imagelist = os.listdir(opt.image_dir)
        coco_eval.params.imgIds = [int(Path(x).stem) for x in imagelist if x.endswith('jpg')]
        coco_eval.evaluate()
        coco_eval.accumulate()
        coco_eval.summarize()
        map_results.update(coco_eval)
        metrics['AP'] += map_results.results['bbox']['AP']
        metrics['AP50'] += map_results.results['bbox']['AP50']
        metrics['AP75'] += map_results.results['bbox']['AP75']
        metrics['APs'] += map_results.results['bbox']['APs']
        metrics['APm'] += map_results.results['bbox']['APm']
        metrics['APl'] += map_results.results['bbox']['APl']
    metrics['AP'] /= 5
    metrics['AP50'] /= 5
    metrics['AP75'] /= 5
    metrics['APs'] /= 5
