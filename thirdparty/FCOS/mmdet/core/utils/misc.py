...
def filter_scores_and_topk(scores, score_thr, topk, results=None):
    ...
    # 输入的scores是[15200,80]
    true_valid_mask = scores > score_thr
    valid_mask = scores > 0.0
    scores = scores[valid_mask]
    # 这里变为[1216000]
    valid_idxs = torch.nonzero(valid_mask) #[1216000,2]往后递减
    # pdb.set_trace()

    num_topk = min(topk, valid_idxs.size(0),scores.size(0))
    # pdb.set_trace()
    # torch.sort is actually faster than .topk (at least on GPUs)
    # scores, idxs = scores.sort(descending=True)
    idxs = torch.arange(scores.size(0), device=scores.device)
    ...
    return scores, labels, keep_idxs, filtered_results, true_valid_mask
